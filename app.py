...
# Page configuration
st.set_page_config(
    page_title="Trading Strategy Dashboard", page_icon="📈", layout="wide"
)
...
cache_folder = Path("./best_worst_data")
...
if parquet_files:
    selected_file = st.sidebar.selectbox("Select Stock Data", parquet_files, index=0)
else:
    selected_file = None
...
# === CACHE FUNCTIONS ===
def get_cache_filepath(stock_name, cache_type="best_worst"):
    """Get the filepath for cached data"""
    return cache_folder / f"{stock_name}_{cache_type}_days.csv"

def cache_exists(stock_name):
    """Check if cached data exists for a stock"""
    cache_file = get_cache_filepath(stock_name)
    return cache_file.exists()

def save_best_worst_to_cache(stock_name, best_days, worst_days):
    """Save best and worst days data to CSV"""
    try:
        # Combine best and worst with a marker column
        best_days_copy = best_days.copy()
        worst_days_copy = worst_days.copy()
        
        best_days_copy['Type'] = 'Best'
        worst_days_copy['Type'] = 'Worst'
        
        # Combine both dataframes
        combined_df = pd.concat([best_days_copy, worst_days_copy], ignore_index=True)
...
def load_best_worst_from_cache(stock_name):
    """Load best and worst days data from CSV"""
    try:
        cache_file = get_cache_filepath(stock_name)
        ...
        # Split back into best and worst
        best_days = combined_df[combined_df['Type'] == 'Best'].drop('Type', axis=1).reset_index(drop=True)
        worst_days = combined_df[combined_df['Type'] == 'Worst'].drop('Type', axis=1).reset_index(drop=True)
...
def calculate_daily_pnl_from_trades(df_processed, starting_capital=100):
    """
    Calculate daily PnL from processed trades with daily starting capital reset.
    Each day starts with $100 capital.
    """
    # Extract only completed trades
    trades = df_processed[df_processed["exit_index"].notna()].copy()
    
    if trades.empty:
        return None
    
    # Add date column for grouping
    trades["entry_date"] = trades.index.date
    
    # Initialize results list
    daily_results = []
    
    # Group by date
    for date, day_trades in trades.groupby("entry_date"):
        daily_capital = starting_capital
        daily_pnl = 0
        
        # Process each trade for the day
        for idx, trade in day_trades.iterrows():
            # Calculate PnL in dollars based on percentage return
            trade_pnl = daily_capital * (trade["pnl_pct"] / 100)
            daily_pnl += trade_pnl
            
            # Update capital for next trade (compounding within the day)
            daily_capital += trade_pnl
        ...
        daily_results.append({
            ...
            "Num Trades": len(day_trades),
            "End Capital": daily_capital
        })
    
    return pd.DataFrame(daily_results)

def get_best_worst_days_from_trades(df_processed, n=15, starting_capital=100):
    """Get best and worst N days by PnL using trade-based calculation"""
    daily_pnl = calculate_daily_pnl_from_trades(df_processed, starting_capital)
...
def calculate_yearly_comparison(df_full, tz, years=[2023, 2024, 2025]):
    ...
    for year in years:
        try:
            ...
            year_end = (
                make_tz_aware(pd.Timestamp(f"{year}-12-31"), tz)
                + pd.Timedelta(days=1)
...
def get_best_worst_trades(trades_df, n=5):
    ...
    best_trades = sorted_trades.head(n)[
        ["entry_price", "exit_price", "position_ml", "pnl", "pnl_pct", "exit_index"]
    ].copy()
    best_trades["Type"] = best_trades["position_ml"].map({1: "Long", -1: "Short"})
    best_trades = best_trades.rename(
        columns={
    ...
    worst_trades = worst_trades.rename(
        columns={
            "entry_price": "Entry Price",
            "exit_price": "Exit Price",
            "pnl": "PnL",
    ...
    while i < len(df):
        ...
        if df.loc[idx, "position_ml"] == 0:
            ...
            while in_trade and j < len(df):
                ...
                if (
                    df.loc[entry_idx, "position_ml"]
                    == df.loc[current_index, "position_ml"]
                ):
                    j += 1
                else:
                    in_trade = False
...
def create_chart(
    ...
    if show_ext and not long_entries.empty:
        ...
        exit_times = long_entries["exit_index"].values
    ...
    if show_ext and not short_entries.empty:
        ...
        exit_times = short_entries["exit_index"].values
    ...
    if show_rsi_ind and rsi_row and "rsi" in df_plot.columns:
        ...
        fig.add_hline(
            y=50, line_dash="dash", line_color="gray", opacity=0.3, row=2, col=1
        )
    # Create custom tick values and labels
    tick_spacing = max(1, len(df_plot) // 20)
    tick_indices = list(range(0, len(df_plot), tick_spacing))
    tick_labels = [
        df_plot.loc[i, "datetime"].strftime("%Y-%m-%d %H:%M") for i in tick_indices
    ]
    # Update layout
    if rsi_row:
        fig.update_xaxes(
...
def create_equity_curve(df, stock_name):
    ...
    df_plot["stock_returns"] = (
        (df_plot["close"] - initial_price) / initial_price
    ) * 100
    # Create the chart
    fig = go.Figure()
    # Equity curve
...
def create_drawdown_chart(df, stock_name):
    ...
    if "per_bar_pnl_ml" in df_plot.columns:
        ...
        df_plot["drawdown_pct"] = (
            df_plot["drawdown"] / running_max.replace(0, 1)
        ) * 100
    else:
        df_plot["drawdown"] = 0
...
if selected_file is not None:
    ...
    end_date = (
        make_tz_aware(end_date_input, tz)
        + pd.Timedelta(days=1)
        - pd.Timedelta(seconds=1)
    )
    ...
    # === BEST AND WORST DAYS ANALYSIS - FULL DATASET (WITH CACHING) ===
    st.subheader("Best & Worst Trading Days Analysis (Trade-Based - Full Dataset)")
    
    # Check if cached data exists
    if cache_exists(stock_name):
        # Load from cache
        with st.spinner("Loading cached best/worst days data..."):
            best_days_yearly, worst_days_yearly = load_best_worst_from_cache(stock_name)
            st.success("✓ Loaded from cache")
    else:
        # Calculate and save to cache
        with st.spinner("Calculating best/worst days for full dataset (this may take a while)..."):
            df_full_processed = process_trades(df_full)
            best_days_yearly, worst_days_yearly = get_best_worst_days_from_trades(
                df_full_processed, n=15, starting_capital=100
            )
            
            # Save to cache
            if best_days_yearly is not None and worst_days_yearly is not None:
                if save_best_worst_to_cache(stock_name, best_days_yearly, worst_days_yearly):
                    st.success("✓ Calculated and saved to cache")
    ...
    if best_days_yearly is not None and worst_days_yearly is not None:
        ...
        with col1:
            ...
            st.dataframe(
                best_days_yearly[["Date", "Daily PnL", "Daily Return (%)", "Num Trades"]].style.format({
                    "Daily PnL": "{:.2f}",
                    "Daily Return (%)": "{:.2f}",
                    "Num Trades": "{:.0f}"
                }),
            ...
            # Download button for best days
            best_days_csv = best_days_yearly.copy()
            best_days_csv["Daily Return (%)"] = best_days_csv["Daily Return (%)"].apply(
                lambda x: f"{x:.2f}%"
            )
            best_days_csv_data = best_days_csv[["Date", "Daily PnL", "Daily Return (%)", "Num Trades"]].to_csv(index=False)
            st.download_button(
                label="Download Best Days CSV",
                data=best_days_csv_data,
                file_name=f"{stock_name}_best_days_trade_based_all_years.csv",
                mime="text/csv",
                key="download_best_days_all_trades",
            )
        
        with col2:
            st.markdown("#### Worst 15 Days (All Years)")
            st.dataframe(
                worst_days_yearly[["Date", "Daily PnL", "Daily Return (%)", "Num Trades"]].style.format({
                    "Daily PnL": "{:.2f}",
                    "Daily Return (%)": "{:.2f}",
                    "Num Trades": "{:.0f}"
                }),
            ...
            # Download button for worst days
            worst_days_csv = worst_days_yearly.copy()
            worst_days_csv["Daily Return (%)"] = worst_days_csv["Daily Return (%)"].apply(
                lambda x: f"{x:.2f}%"
            )
            worst_days_csv_data = worst_days_csv[["Date", "Daily PnL", "Daily Return (%)", "Num Trades"]].to_csv(index=False)
            st.download_button(
                label="Download Worst Days CSV",
                data=worst_days_csv_data,
                file_name=f"{stock_name}_worst_days_trade_based_all_years.csv",
                mime="text/csv",
                key="download_worst_days_all_trades",
            )
    
    # === BEST AND WORST DAYS ANALYSIS - DATE FILTERED (NO CACHING) ===
    st.subheader("Best & Worst Trading Days Analysis (Trade-Based - Filtered Date Range)")
    best_days_filtered, worst_days_filtered = get_best_worst_days_from_trades(df_processed, n=15, starting_capital=100)
    ...
    if best_days_filtered is not None and worst_days_filtered is not None:
        ...
        with col1:
            ...
            st.dataframe(
                best_days_filtered[["Date", "Daily PnL", "Daily Return (%)", "Num Trades"]].style.format({
                    "Daily PnL": "{:.2f}",
                    "Daily Return (%)": "{:.2f}",
                    "Num Trades": "{:.0f}"
                }),
            ...
            # Download button
            best_days_filtered_csv = best_days_filtered.copy()
            best_days_filtered_csv["Daily Return (%)"] = best_days_filtered_csv["Daily Return (%)"].apply(
                lambda x: f"{x:.2f}%"
            )
            best_days_filtered_csv_data = best_days_filtered_csv[["Date", "Daily PnL", "Daily Return (%)", "Num Trades"]].to_csv(index=False)
            st.download_button(
                label="Download Best Days CSV",
                data=best_days_filtered_csv_data,
                file_name=f"{stock_name}_best_days_trade_based_{start_date.date()}_to_{end_date.date()}.csv",
                mime="text/csv",
                key="download_best_days_filtered_trades",
            )
        
        with col2:
            st.markdown("#### Worst 15 Days (Selected Period)")
            st.dataframe(
                worst_days_filtered[["Date", "Daily PnL", "Daily Return (%)", "Num Trades"]].style.format({
                    "Daily PnL": "{:.2f}",
                    "Daily Return (%)": "{:.2f}",
                    "Num Trades": "{:.0f}"
                }),
            ...
            # Download button
            worst_days_filtered_csv = worst_days_filtered.copy()
            worst_days_filtered_csv["Daily Return (%)"] = worst_days_filtered_csv["Daily Return (%)"].apply(
                lambda x: f"{x:.2f}%"
            )
            worst_days_filtered_csv_data = worst_days_filtered_csv[["Date", "Daily PnL", "Daily Return (%)", "Num Trades"]].to_csv(index=False)
            st.download_button(
                label="Download Worst Days CSV",
                data=worst_days_filtered_csv_data,
                file_name=f"{stock_name}_worst_days_trade_based_{start_date.date()}_to_{end_date.date()}.csv",
                mime="text/csv",
                key="download_worst_days_filtered_trades",
            )
    ...
    if not trades.empty:
        ...
        with col1:
            ...
            if best_trades is not None:
                st.dataframe(
                    best_trades[
                        ["Type", "Entry Price", "Exit Price", "PnL", "PnL %"]
                    ].style.format(
                        {
                            "Entry Price": "{:.2f}",
                            "Exit Price": "{:.2f}",
    ...
    if monthly_perf_2025 is not None and not monthly_perf_2025.empty:
        ...
        # Download button for monthly performance with percentage sign
        monthly_csv_export = monthly_sorted.copy()
        monthly_csv_export["Return (%)"] = monthly_csv_export["Return (%)"].apply(
            lambda x: f"{x:.2f}%"
        )
        monthly_csv_data = monthly_csv_export.to_csv(index=False)
        st.download_button(
            label="Download Monthly Performance 2025 CSV",
            data=monthly_csv_data,
            file_name=f"{stock_name}_monthly_performance_2025.csv",
            mime="text/csv",
            key="download_monthly_2025",
        )
        ...
        if monthly_perf_all is not None and not monthly_perf_all.empty:
            ...
            monthly_perf_all["Month_Name"] = pd.to_datetime(
                monthly_perf_all["Month"]
            ).dt.strftime("%b")
            
            pivot_table = monthly_perf_all.pivot_table(
                values="Return (%)", index="Year", columns="Month_Name", aggfunc="sum"
            )
    ...
    if not trades.empty:
        ...
        trade_details["position_type"] = trade_details["position_ml"].map(
            {1: "Long", -1: "Short"}
        )
        trade_details = trade_details.rename(
            columns={
                "entry_price": "Entry Price",
                "exit_price": "Exit Price",
                "pnl": "PnL",
        ...
        st.dataframe(
            trade_details[
                ["Type", "Entry Price", "Exit Price", "PnL", "PnL %", "Exit Time"]
            ],
            use_container_width=True,
        )
        
        # Prepare CSV with percentage sign
        csv_export = trade_details[
            ["Type", "Entry Price", "Exit Price", "PnL", "PnL %", "Exit Time"]
        ].copy()
        csv_export["PnL %"] = csv_export["PnL %"].apply(lambda x: f"{x:.2f}%")
