...
# Function to calculate daily PnL from equity changes
def calculate_daily_pnl(df):
    """Calculate daily PnL and percentage returns from equity column"""
    ...
    daily_equity = df_daily.groupby("date")["equity"].agg(["first", "last"]).reset_index()
    daily_equity.columns = ["Date", "Start Equity", "End Equity"]
    
    # Calculate Daily PnL from equity change
    daily_equity["Daily PnL"] = daily_equity["End Equity"] - daily_equity["Start Equity"]
    
    # Calculate percentage return
    daily_equity["Daily Return (%)"] = (
        daily_equity["Daily PnL"] / daily_equity["Start Equity"]
    ) * 100
    
    # Keep only Date, Daily PnL, and Daily Return (%)
    daily_data = daily_equity[["Date", "Daily PnL", "Daily Return (%)"]]
...
def get_best_worst_days(df, n=15):
    """Get best and worst N days by PnL with percentage returns"""
...
if selected_file is not None:
    ...
    if best_days_yearly is not None and worst_days_yearly is not None:
        ...
        with col1:
            ...
            st.dataframe(
                best_days_yearly.style.format({"Daily PnL": "{:.2f}", "Daily Return (%)": "{:.2f}"}),
                use_container_width=True,
            )

            # Download button for best days with percentage sign
            best_days_csv = best_days_yearly.copy()
            best_days_csv["Daily Return (%)"] = best_days_csv["Daily Return (%)"].apply(
                lambda x: f"{x:.2f}%"
            )
            best_days_csv_data = best_days_csv.to_csv(index=False)
            st.download_button(
                label="Download Best Days CSV",
                data=best_days_csv_data,
                file_name=f"{stock_name}_best_days_all_years.csv",
                mime="text/csv",
                key="download_best_days_all",
            )

        with col2:
            st.markdown("#### Worst 15 Days (All Years)")
            st.dataframe(
                worst_days_yearly.style.format({"Daily PnL": "{:.2f}", "Daily Return (%)": "{:.2f}"}),
                use_container_width=True,
            )

            # Download button for worst days with percentage sign
            worst_days_csv = worst_days_yearly.copy()
            worst_days_csv["Daily Return (%)"] = worst_days_csv["Daily Return (%)"].apply(
                lambda x: f"{x:.2f}%"
            )
            worst_days_csv_data = worst_days_csv.to_csv(index=False)
            st.download_button(
                label="Download Worst Days CSV",
                data=worst_days_csv_data,
                file_name=f"{stock_name}_worst_days_all_years.csv",
                mime="text/csv",
                key="download_worst_days_all",
            )
    ...
    if best_days_filtered is not None and worst_days_filtered is not None:
        ...
        with col1:
            ...
            st.dataframe(
                best_days_filtered.style.format({"Daily PnL": "{:.2f}", "Daily Return (%)": "{:.2f}"}),
                use_container_width=True,
            )

            # Download button for best days filtered with percentage sign
            ...
            best_days_filtered_csv_data = best_days_filtered_csv.to_csv(index=False)
            st.download_button(
                label="Download Best Days CSV",
                data=best_days_filtered_csv_data,
                file_name=f"{stock_name}_best_days_{start_date.date()}_to_{end_date.date()}.csv",
                mime="text/csv",
                key="download_best_days_filtered",
            )

        with col2:
            st.markdown("#### Worst 15 Days (Selected Period)")
            st.dataframe(
                worst_days_filtered.style.format({"Daily PnL": "{:.2f}", "Daily Return (%)": "{:.2f}"}),
                use_container_width=True,
            )

            # Download button for worst days filtered with percentage sign
            ...
            worst_days_filtered_csv_data = worst_days_filtered_csv.to_csv(index=False)
            st.download_button(
                label="Download Worst Days CSV",
                data=worst_days_filtered_csv_data,
                file_name=f"{stock_name}_worst_days_{start_date.date()}_to_{end_date.date()}.csv",
                mime="text/csv",
                key="download_worst_days_filtered",
            )
